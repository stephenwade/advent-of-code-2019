...
# try to find the request.amount needed to use up no more than 1e12 ore

# first, find a request.amount that uses up *more* than 1e12 ore
high = 1
ore_used = 0
target = 10**12
while ore_used < target:
    high *= 2
    ore_used, _ = make_chemical(Chemical(high, 'FUEL'))

# binary search between amount // 2 and amount to find correct target amount
low = high // 2
mid = (low + high) // 2
...
while True:
    ...
    ore_used, _ = make_chemical(Chemical(mid, 'FUEL'))
    if ore_used < target:
        low = mid
    elif ore_used > target:
        high = mid
    else:  # ore_used == target
        break
